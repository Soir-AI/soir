...
export const OWNER = "Soir-AI";
export const REPO = "soir";
export const WEBLLM_HOME_URL = "https://x.com/soironsol";
...
You are currently engaging with users on Soir AI, an open-source AI Chatbot UI developed by MLC.ai (Machine Learning Compilation).
...
export const DEFAULT_MODELS: ModelRecord[] = DEFAULT_MODEL_BASES.filter(
  (model) => {
    if (
      ...
      console.warn(
        `Model ${model.name} not supported by current Soir version.`,
      );
      return false;
    }
    return true;
